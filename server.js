...
直接输出文案内容，不要加任何说明。`;
  ...
  try {
    ...
    if (!apiKey) {
      // 没有 API key 时返回模拟文案作为降级方案
      return res.json({ content: getFallbackText(product, style, keyword, audience) });
    }
    ...
    const content = data.choices?.[0]?.message?.content || getFallbackText(product, style, keyword, audience);
    res.json({ content });
  } catch (err) {
    console.error('AI 调用失败:', err.message);
    res.json({ content: getFallbackText(product, style, keyword, audience) });
  }
});

// 降级文案（API 不可用时使用）
function getFallbackText(product, style, keyword, audience) {
  const templates = {
  ...
  return templates[style] || templates['种草'];
}
